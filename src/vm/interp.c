/* Automatically generated from Squeak on 28 September 2010 2:21:16 pm 
   by VMMaker 3.11.12
 */
...
#define MethodCacheClosedValue 7
...
#define SelectorCannotInterpretInLayer 50
#define SelectorCannotReturn 21
#define SelectorDoesNotUnderstand 20
#define SelectorMustBeBoolean 25
#define SelectorPerformWithoutLayers 51
...
sqInt findNewMethodWithoutLayersInClass(sqInt class);
...
sqInt lookForinDictionary(sqInt anObject, sqInt dictionary);
sqInt lookupMethodInClass(sqInt class);
sqInt lookupMethodInLayer(sqInt layer);
...
sqInt primitiveCloseCacheEntries(void);
...
sqInt primitiveOpenCacheEntries(void);
...
sqInt primitivePerformWithoutLayersWithArgs(void);
...
void *primitiveTable[580] = {
	...
	/* 576*/ (void *)primitiveOpenCacheEntries,
	/* 577*/ (void *)primitiveCloseCacheEntries,
	/* 578*/ (void *)primitivePerformWithoutLayersWithArgs,
 0 };
usqInt memory;
void* showSurfaceFn;
...
register struct foo * foo = &fum;
	...
	probe = hash & MethodCacheMask;
	if ((foo->methodCache[probe + MethodCacheClosedValue]) > 0) {
		ok = 0;
		goto l1;
	}
...
l1:	/* end lookupInMethodCacheSel:class: */;
	if (!(ok)) {
		...
		layer = longAt((proc + BaseHeaderSize) + (7 << ShiftForWord));
		lookupMethodInLayer(layer);
		addNewMethodToCache();
	}
}
...
l1:	/* end sizeBitsOf: */;
	...
	if ((anObject & 1)) {
		index = (mask & ((anObject >> 1))) + SelectorStart;
	} else {
		index = (mask & ((((usqInt) (longAt(anObject))) >> 17) & 4095)) + SelectorStart;
	}
	wrapAround = 0;
	while (1) {
		nextObject = longAt((dictionary + BaseHeaderSize) + (index << ShiftForWord));
		if (nextObject == foo->nilObj) {
			return foo->nilObj;
		}
		if (nextObject == anObject) {
			dictArray = longAt((dictionary + BaseHeaderSize) + (MethodArrayIndex << ShiftForWord));
			newDict = longAt((dictArray + BaseHeaderSize) + ((index - SelectorStart) << ShiftForWord));
			if (newDict == foo->nilObj) {
				return foo->trueObj;
			}
			return newDict;
		}
...
register struct foo * foo = &fum;
	...
	currentLayer = layer;
	while (currentLayer != foo->nilObj) {
		objDict = longAt((currentLayer + BaseHeaderSize) + (1 << ShiftForWord));
		if (objDict != foo->nilObj) {
			dictionary = lookForinDictionary(foo->receiverbis, objDict);
			if (!(dictionary == foo->nilObj)) {
				if (dictionary == foo->trueObj) {
					found = longAt((foo->specialObjectsOop + BaseHeaderSize) + (SelectorPerformWithoutLayers << ShiftForWord));
					if (foo->messageSelector == found) {
						return lookupMethodInClass(foo->lkupClass);
					}
					/* begin pushRemappableOop: */
					foo->remapBuffer[foo->remapBufferCount += 1] = currentLayer;
					createActualMessageTo(foo->lkupClass);
					/* begin popRemappableOop */
					oop = foo->remapBuffer[foo->remapBufferCount];
					foo->remapBufferCount -= 1;
					currentLayer = oop;
					foo->messageSelector = longAt((foo->specialObjectsOop + BaseHeaderSize) + (SelectorCannotInterpretInLayer << ShiftForWord));
					currentLayer = longAt((currentLayer + BaseHeaderSize) + (0 << ShiftForWord));
					return lookupMethodInLayer(currentLayer);
				}
				/* begin lookupMethodInDictionary: */
				/* begin fetchWordLengthOf: */
				/* begin sizeBitsOf: */
		...
		classDict = longAt((currentLayer + BaseHeaderSize) + (2 << ShiftForWord));
		if (classDict != foo->nilObj) {
			dictionary = lookForinDictionary(foo->lkupClass, classDict);
			if (!(dictionary == foo->nilObj)) {
				if (dictionary == foo->trueObj) {
					found = longAt((foo->specialObjectsOop + BaseHeaderSize) + (SelectorPerformWithoutLayers << ShiftForWord));
					if (foo->messageSelector == found) {
			...
			l3:	/* end sizeBitsOf: */;
				...
				if ((foo->messageSelector & 1)) {
					index1 = (mask1 & ((foo->messageSelector >> 1))) + SelectorStart;
				} else {
					index1 = (mask1 & ((((usqInt) (longAt(foo->messageSelector))) >> 17) & 4095)) + SelectorStart;
				}
				wrapAround1 = 0;
				while (1) {
					nextSelector1 = longAt((dictionary + BaseHeaderSize) + (index1 << ShiftForWord));
					if (nextSelector1 == foo->nilObj) {
						found = 0;
						goto l4;
					}
					if (nextSelector1 == foo->messageSelector) {
						methodArray1 = longAt((dictionary + BaseHeaderSize) + (MethodArrayIndex << ShiftForWord));
						foo->newMethod = longAt((methodArray1 + BaseHeaderSize) + ((index1 - SelectorStart) << ShiftForWord));
						if (((((usqInt) (longAt(foo->newMethod))) >> 8) & 15) >= 12) {
							/* begin primitiveIndexOf: */
							primBits1 = (((usqInt) (longAt((foo->newMethod + BaseHeaderSize) + (HeaderIndex << ShiftForWord)))) >> 1) & 268435967;
							foo->primitiveIndex = (primBits1 & 511) + (((usqInt) primBits1) >> 19);
							if (foo->primitiveIndex > MaxPrimitiveIndex) {
								foo->primitiveIndex = 0;
							}
						} else {
...
/*	two args: selector and class -> close a specific entry */
/*	no arg -> close all entries */
...
register struct foo * foo = &fum;
	...
	if (foo->argumentCount == 2) {
		aClass = longAt(foo->stackPointer);
		/* begin pop: */
		foo->stackPointer -= 1 * BytesPerWord;
		aSelector = longAt(foo->stackPointer);
		/* begin pop: */
		foo->stackPointer -= 1 * BytesPerWord;

		/* shift drops two low-order zeros from addresses */

		hash = aSelector ^ aClass;

		/* first probe */

		probe = hash & MethodCacheMask;
		n = 1 + (foo->methodCache[probe + MethodCacheClosedValue]);
		foo->methodCache[probe + MethodCacheClosedValue] = n;
	} else {
		probe = 0;
		for (i = 1; i <= MethodCacheEntries; i += 1) {
			n = 1 + (foo->methodCache[probe + MethodCacheClosedValue]);
...
/*	two args: selector and class -> open a specific entry */
/*	no arg -> open all entries */
...
register struct foo * foo = &fum;
	...
	if (foo->argumentCount == 2) {
		...
		probe = hash & MethodCacheMask;
		n = (foo->methodCache[probe + MethodCacheClosedValue]) - 1;
		foo->methodCache[probe + MethodCacheClosedValue] = n;
	} else {
		probe = 0;
		for (i = 1; i <= MethodCacheEntries; i += 1) {
...
register struct foo * foo = &fum;
	...
	sel = longAt((foo->specialObjectsOop + BaseHeaderSize) + (SelectorPerformWithoutLayers << ShiftForWord));
	/* begin push: */
	longAtput(sp = foo->stackPointer + BytesPerWord, sel);
	foo->stackPointer = sp;
	/* begin primitiveFlushCacheSelective */
	selector = longAt(foo->stackPointer);
	probe = 0;
	for (i = 1; i <= MethodCacheEntries; i += 1) {
		if ((foo->methodCache[probe + MethodCacheSelector]) == selector) {
...
l1:	/* end fetchClassOf: */;
	/* begin primitivePerformWithoutLayersAt: */
	argumentArray = longAt(foo->stackPointer);
	if (!(((argumentArray & 1) == 0) && (((((usqInt) (longAt(argumentArray))) >> 8) & 15) == 2))) {
		/* begin primitiveFail */
		foo->successFlag = 0;
	...
	while (index <= arraySize) {
		/* begin push: */
		longAtput(sp3 = foo->stackPointer + BytesPerWord, longAt((argumentArray + BaseHeaderSize) + ((index - 1) << ShiftForWord)));
		foo->stackPointer = sp3;
		index += 1;
	}
	foo->argumentCount = arraySize;
	findNewMethodWithoutLayersInClass(lookupClass);
	if (((((usqInt) (longAt(foo->newMethod))) >> 8) & 15) >= 12) {
		/* begin success: */
		foo->successFlag = (((((usqInt) (longAt((foo->newMethod + BaseHeaderSize) + (HeaderIndex << ShiftForWord)))) >> 25) & 15) == foo->argumentCount) && foo->successFlag;
	}
...
l5:	/* end primitivePerformWithoutLayersAt: */;
}
